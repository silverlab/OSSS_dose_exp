...
if __name__ == "__main__":
    ...
    # try
    # import pyschopy.monitor as calib
    ...
    win = visual.Window(fullscr=params.fullscreen,
                        monitor=params.monitor, units="deg", screen=params.screen)
    ...
    for trial_idx, this_trial in enumerate(trial_list):
        ...
        # if this_trial.block_type == 'A':
        if this_trial.block_type == 'A':
            this_trial.finalize_fix(params, bank, staircaseA, [trial_idx])
            this_trial.finalize_fix(
                params, bank, staircaseA, other_contrast[trial_idx])

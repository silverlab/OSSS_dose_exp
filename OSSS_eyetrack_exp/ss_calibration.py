...
def create_indices(h, w):
    ...
    for j in range(4):
        i = j+1
        indices = np.concatenate((indices, [(0,y/i), (0,-y/i), (x/i,0), (-x/i,0), (x/i, y/i), (-x/i, -y/i), (-x/i,y/i),(x/i,-y/i)]), axis=0)
    return indices
...
def run_calibration(subject_initials, win, data_dir):
    ...
    if(getEYELINK().isConnected() and not getEYELINK().breakPressed()):
        ...
        getEYELINK().sendMessage(start_message)
            ...
            ### AFTER PASTE
        df_calib = pd.DataFrame(columns=['last_gaze','all_gaze', 'start_time', 'end_time', 'fix'])
        for fix_position in indices:
            #print fix_position
            circle = visual.Circle(win, radius = 0.25, lineWidth=1, fillColor='white', lineColor='white', edges=32, pos=(fix_position[1], fix_position[0]))
            ...
            new_row = pd.DataFrame({'last_gaze':[last_gaze],'all_gaze':[gaze_collection], 'start_time':[newFixTime], 'end_time':[endTime], 'fix':[(fix_position[0], fix_position[1])]}, index=[0])
    ...
    df_calib.to_csv(data_dir + '/' + subject_initials+'_calibration_coords.csv', sep='\t')

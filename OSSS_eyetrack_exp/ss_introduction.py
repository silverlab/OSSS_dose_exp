...
#    core.wait(secs=5)
    ...
    dummy_num = 50
    # but want to change a couple of things
    dummy_params = copy.deepcopy(params)
    ...
    for i in range(3):
        ...
        other_contrast0 = np.ones(len(dummy_trials)) * \
            dummy_params.fix_target_start
        ...
        while correct_response < 5:
            ...
            dthis_trial.finalize_stim(
                dummy_params, bank0, staircase0, other_contrast0[dtrial_idx])
            dthis_trial.stimulus()
            dthis_trial.finalize_fix(
                dummy_params, bank0, staircase0, [dtrial_idx])
            dthis_trial.finalize_fix(
                dummy_params, bank0, staircase0, other_contrast0[dtrial_idx])

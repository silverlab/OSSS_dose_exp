...
if(getEYELINK().isConnected() and not getEYELINK().breakPressed()):
    ...
    for fix_position in indices:
        #print fix_position
        circle = visual.Circle(win, radius = 5, lineWidth=1, fillColor='white', lineColor='white', edges=32, pos=(fix_position[1], fix_position[0]))
        ...
        new_row = pd.DataFrame({'gaze':[gaze_collection], 'start_time':[newFixTime], 'end_time':[endTime], 'fix':[fix_position.tolist()]}, index=[0])

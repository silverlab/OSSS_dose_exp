...
#list_data_dir = subject_folder + ('/%s' %time.strftime('%m%d%Y')+str(run_num)) = 
...
#        condition_types.remove('PSH')
    while condition_types != []:
        if run_list[len(run_list)-1][:-1] == 'PSH': #our run list is a collection of strings
            #since our last condition was parallel, we want to display orthogonal or no surround
        ...
        elif run_list[len(run_list)-1][:-1] == 'PSV': #our run list is a collection of strings
            #since our last condition was parallel, we want to display orthogonal or no surround
            pick = random.randint(0,1)
        ...
        elif run_list[len(run_list)-1][:-1] == 'NSH': #our run list is a collection of strings
            #since our last condition was parallel, we want to display orthogonal or no surround
            pick = random.randint(0,1)
            if pick == 1 and 'OSV' in condition_types:
                run_list.append('OSV'+run_num)
        ...
        elif run_list[len(run_list)-1][:-1] == 'NSV': #our run list is a collection of strings
            #since our last condition was parallel, we want to display orthogonal or no surround
            pick = random.randint(0,1)
            if pick == 1 and 'OSH' in condition_types:
                run_list.append('OSH'+run_num)
        ...
        elif run_list[len(run_list)-1][:-1] == 'OSH': #our run list is a collection of strings
            #since our last condition was parallel, we want to display orthogonal or no surround
            pick = random.randint(0,1)
        ...
        elif run_list[len(run_list)-1][:-1] == 'OSV': #our run list is a collection of strings
            #since our last condition was parallel, we want to display orthogonal or no surround
            pick = random.randint(0,1)
...
#Initialize params from file:
    ...
    if int(block[-1]) == 1 and int(run_num) == 1:
        ...
        if start_contrasts[block[:3]] != '':
            ...
            params.start_target_orthog_contrastA = int(start_contrasts[block[:3]])
    ...
    win = visual.Window(fullscr= params.fullscreen, monitor=mon, units="deg",screen=params.screen)    #Initialize all the instances of stimulus bits and pieces for reuse: 
    ...
    if block_num == 1 and intro_ON == True:
        ...
        ss_introduction.Run_Intro_Trials(win, params, intro_trials, subject, list_data_dir)
    ...
    getEYELINK().sendMessage("DISPLAY_COORDS  0 0 %d %d" %(win.size[0] - 1, win.size[1] - 1))
    ...
    if(getEYELINK().isConnected() and not getEYELINK().breakPressed()):
        block_contrast = ss_run_eyetrack.run_trials(win, params, trial_list, bank, f, staircaseA, staircaseB, other_contrast)

...
class GetFromSimpleGui(wx.Dialog):
    ...
    def __init__(self, parent, id, title, combo_choices=['No Choices Given']):
        ...
        wx.StaticText(
            self, -1, 'How many blocks (for each condition):', pos=(10, 200))
        # Add the subj id text box, drop down menu, radio buttons
        self.sub_textbox = wx.TextCtrl(self, -1, pos=(100, 18), size=(150, -1))
        self.run_num_textbox = wx.TextCtrl(
            self, -1, pos=(100, 53), size=(50, -1))
        self.parallel_h_textbox = wx.TextCtrl(
            self, -1, pos=(100, 90), size=(50, -1))
        self.parallel_v_textbox = wx.TextCtrl(
            self, -1, pos=(170, 90), size=(50, -1))
        self.orthogonal_h_textbox = wx.TextCtrl(
            self, -1, pos=(100, 125), size=(50, -1))
        self.orthogonal_v_textbox = wx.TextCtrl(
            self, -1, pos=(170, 125), size=(50, -1))
        self.no_surr_h_textbox = wx.TextCtrl(
            self, -1, pos=(100, 160), size=(50, -1))
        self.no_surr_v_textbox = wx.TextCtrl(
            self, -1, pos=(170, 160), size=(50, -1))
        self.replay_contrast = None

        # Radio Buttons for the number of tasks
        self.rb_block1 = wx.RadioButton(
            self, -1, '1', (25, 225), style=wx.RB_GROUP)
...
class GetFromGui(wx.Dialog):
    ...
    def __init__(self, parent, id, title, combo_choices=['No Choices Given']):
        ...
        self.replayStat = wx.StaticText(
            self, -1, 'No Replay Contrast Set', pos=(98, 230))
        # Add the subj id text box, drop down menu, radio buttons
    ...
    def OnReplay(self, event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            self.replay_contrast = staircase_from_file(
                file_name, self.replay_id)
...
def start_data_file(subject_id, sub_dir):
    ...
    i = 1
    this_data_file = 'SS_%s_%s_%s.psydat' % (
        subject_id, time.strftime('%m%d%Y'), i)

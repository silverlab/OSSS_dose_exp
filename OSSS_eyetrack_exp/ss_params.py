...
p = dict(
    paradigm = 'rapid_fire',#'rapid_fire',     # 'block' for the fmri block design
                                 ...
                                 #trials. 
    surround_contrast = 0.8*1.14,     # relative contrast, Default: 0.75
    monitor =   '404_CRT_NEWNEW', #'alt_Test', #'NEC2',     # 'testMonitor', '404_CRT'
    ...
    start_target_contrastA = 0.28,#0.2*1.14, #0.175, # Where to start the staircase.
    start_target_orthog_contrastA = 0.28,# 0.2*1.14, #0.175, #Where to start the staircase for orthogonal condition.  This will make sure that staircases converge on threshold.
    start_target_contrastB = 0.28,# 0.2*1.14, #0.175, # Where to start the staircase.
    ...
    annulus_contrast = 0.35*1.14,     # relative contrast, Default: 0.75
    ...
    spatial_freq = 1.1, #0.06875, #1.1,          # cycles/deg, Default: 1.1
    spatial_phase = 0,           # seconds, Default: 0
    temporal_freq = 4, #0.0078125, #4,           # Hz, Default: 4
    temporal_phase = 0,          # seconds, Default: 0
    stimulus_duration = 0.75, #192,    # seconds, Default: 0.75
    fixation_duration = 0.1,     # seconds, Default: 0.1
    response_duration = 0.1,     # seconds, Default: 0.9
    feedback_duration = 0.25,    # seconds, Default: 0.25
    fixation_size = 0.8,         # deg of visual angle, Standard: 0.8
    contrast_increments = 20,    # How many steps from the lowest to the highest
                                 #contrast, Standard: 15 
    stupidity_check = 'in here',
       #
    targetA_contrast_max = 0.35*1.14,
    #targetA_contrast_min = 0.001,
    targetA_contrast_min = 0.05,
    targetB_contrast_max = 0.35*1.14,
    targetB_contrast_min = 0.05,
    fix_target_max = 1,          #
    fix_target_min = 0.5,       #
    trials_per_block = 80,        #
    num_blocks = 1,             # Number of trials will be num_blocks *
                                 # trials_per_block 
    dummy_blocks = 5,            # In 'block' mode, the number of dummy blocks

...
def do_trial(trial_idx, this_trial, win, staircaseA, staircaseB, other_contrast, exp_clock, params, bank,f):
    ...
    eye_message ="record_status_message 'Trial %d %s'"%(trial_idx, this_trial.block_type)  #!!! change trial_condition
    ...
    check_saccade = this_trial.stimulus() # THIS SHOWS THE STIMULUS -> SO WANT TO COMMUNICATE WITH EYETRACKER IN THIS LOOP
...
# !!! UNCOMMENT THIS WITH HUMAN SUBJECT
    ...
    for trial_idx, this_trial in enumerate(trial_list):
        ...
        while 1:
            ret_value = do_trial(trial_idx, this_trial, win, staircaseA, staircaseB, other_contrast, experiment_clock, params, bank,f)
            endRealTimeMode()
        ...
        ### Want to get the last contrast to start the next block (of the same condition), of that trial_clock

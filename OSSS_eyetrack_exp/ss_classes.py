...
class Params(object):
    ...
    def __setattr__(self, name, value):
        ...
        if name == '_dont_touch':
            ...
            raise ValueError(
                "Parameter %s is protected, please don't touch!" % name)
        else:
            super.__setattr__(self, name, value)
    ...
    def save(self, f, open_and_close=False):
        ...
        if open_and_close:
            ...
            for k in self.__dict__.keys():
                # Exclude 'private' variables ('_dont_touch' and
                if k[0] != '_':
                              #'_replay'):
                    f.write('# %s : %s \n' % (k, self.__dict__[k]))
            f.close()
...
class Event(object):
    ...
    def __call__(self, **kwargs):
        ...
        # In the simplest case, just clear the screen completely at each
        # refresh:
        clock = core.Clock()
        t = 0
...
class Staircase(object):
    ...
    def update(self, correct, check_saccade):
        """
...
class StimulusBank():
    ...
    def __init__(self, win, params, tex_res=256):
        ...
        # This is the central area, between the inner surround and the
        # fixation:
...
#                                 np.deg2rad(target_ori))] = -1
    ...
    def __call__(self):
        ...
        saccade = 0
        while t < self.duration:  # Keep going for the duration
            #            self.clearEvents(eventType='keyboard')
            t = clock.getTime()
            # want to listen to the eyetracker
            eye_check = getEYELINK().getNextData

            if eye_check == 5 and saccade == 0:  # STARTSACC
                saccade = 1
            if eye_check == 3:  # STARTBLINK
                blink = 1
                saccade = 0
...
class Response(Event):
    ...
    def __init__(self, params, keys=['num_1', 'num_4', 'num_3', 'num_6'], duration=None):
        """
    ...
    def __call__(self):
        ...
        # Instead loop until we recieve a key press:
        while key_was_pressed == None:
            t = clock.getTime()
...
class Trial(Event):
    ...
    def finalize_stim(self, params, bank, staircase, other_contrast):
        ...
        if self.params.task == 'Annulus':
            ...
            if self.target_loc in [0]:
                self.correct_key = 'num_1'
            elif self.target_loc in [1]:
                self.correct_key = 'num_4'
            elif self.target_loc in [3]:
                self.correct_key = 'num_3'
            elif self.target_loc in [2]:
                self.correct_key = 'num_6'  # '6' #
    ...
    def save_time(self, time, type):
        if type == 0:  # start time
            self.start_time = time
        elif type == 1:  # end time
            self.end_time = time
    ...
    def save(self, f, insert_header=False):
        ...
        if insert_header:
            ...
            f.write('Response, ')
            f.write('Correct, ')
            # Liz Comment: Adding two new things to save in each trial: trial start time and trial end time -> to be used to
            # synch with eye tracking data
            f.write('Trial start time, ')
            f.write('Trial end time, ')
            f.write('Response time\n')

        # Annulus target contrast
        f.write('%s, ' % self.stimulus.nominal_target_co)
        # Fixation target contrast
        f.write('%s, ' % (self.stimulus.fixation_target.color[0]))
        f.write('%s, ' % self.stimulus.target_loc)  # Annulus target location
        # Fixation target location
        f.write('%s, ' % self.stimulus.fix_target_loc)
        f.write('%s, ' % self.response.key)  # Response
        # Record no-response trials as a -1: # Correct
        if self.response.correct is None:
            f.write('%s, ' % -1)
        else:
            f.write('%s, ' % self.response.correct)
        # Trial start time -> need to implement
        f.write('%s, ' % self.start_time)
        # Trial end time -> also need to implement
        f.write('%s, ' % self.end_time)
        f.write('%s\n' % self.response.response_time)  # Response time
...
#                              )
    ...
    # If the input is not valid throw and error:
        ...
        raise ValueError(
            "Need to set params.paradigm to a valid value (in ss_params.py")

...
if __name__ == "__main__":
    ...
    dataFrameSaved = pd.ExcelWriter(sub_folder+'/proccessed/'+ current_subject+'_'+date+'_session'+session+'_master_file.xlsx', engine='xlsxwriter')
    ...
    for fn in os.listdir(sub_folder):
        ...
        if fn.endswith('.csv') == True:
            ...
            for index, row in data.iterrows(): # we are iterating through the whole data frame
                ...
                if isinstance(row['time'], basestring) == True and row['time'].isdigit() == False: # then we have a string message from eyelink
            ...
            for i in blinked_time:
                df_data.loc[df_data.time == i, 'Blink'] = 1 #if blink happening = 1 based on blink timepoints

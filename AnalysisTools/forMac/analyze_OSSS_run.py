...
def correct_gaze():
    ...
    for i in range(len(df_calib)):
        ...
        gaze = ast.literal_eval(gaze) # string manipulation?, same output as gaze above
        ...
        fix_y = (1)*int(fx[0]) + (880/2) #taking ground truth fixation points and adding half of screen size
        fix_x = (-1)*int(fx[1]) + (1150/2)#taking ground truth fixation points and adding half of screen size
    ...
    for i in range(len(gaze_at_fix)):
        ...
        new_row = pd.DataFrame({'fix_x':[fix_x], 'fix_y':[fix_y], 'x_dis':[x_dis], 'y_dis':[y_dis]}, index=[0]) # saving GT and displacement
...
def plot_eyetrack(dir_name, subject, date, all_runs): # doesn't finish this function plots raw ET data showing inner and outer annulus
    ...
    ### !!! Change this back
        ...
        df_stim_on = df_stim_on[df_stim_on.Stimuli_On == 1] # subject data with stimuli on
        ...
        #Preliminary efforts to weed out bad trials. Proportion of time subject not properly fixating (# bad samples/ # total samples)
        total = len(subject_data.parse(sheet)) # all samples stimuli present
        samplesDiscarded = len(subject_data.parse(sheet))-len(df_stim_on) # discarded samples (stimuli not on)
        outsideFix = len(df_data.time[df_data.Out_Bounds_stim_On==1]) # stimulus on and NOT fixating properly 
        pTimeBad =  outsideFix /len(df_stim_on) # proportion of time NOT fixating properly 
...
#     plt.ylim([0, 800])
    ...
    plt.savefig(dir_name + '/proccessed/' + subject+'_'+date +'_session' + session+'_eyePosition.png')


def plot_psychophysical_performance(subject, date, dir_name, num_blocks):
    output = open(dir_name + '/proccessed/' + subject+'_'+date+ '_session' + session + '_ContrastThresholds.txt', 'a') # Save Thresholds into a .txt file
    ...
    EyeData = pd.ExcelFile(dir_name+'/proccessed/'+subject+'_'+date+'_session' + session+'_master_file.xlsx')
    ...
    for fn in os.listdir(dir_name):
        if fn.endswith('psydat')==True:
            ...
            if (int(fn[9:10]) == 1):
                ...
                df_eye = EyeData.parse(subject+'_'+condition) #selecting sheet based from master excelfile CHECK WHAT IS GOING ON!
                # remove blink eye movements
                df_eye = df_eye[df_eye.Blink == 0]

                df_broken_fix = df_eye.copy()
                df_broken_fix = df_broken_fix[df_broken_fix.In_Bound == 1] ### LIZ: double check this


                invalid_trials = df_broken_fix['trial'].tolist() #removing bad trials?
                ...
                while l[0]=='#':
                    ...
                    l = file_read.readline()
                data_rec = csv2rec(dir_name + '/'+ file_name) # imports psydat file
                ...
                if num_blocks > 1:
                    for i in range(2, num_blocks+1):
                        ...
                        df_broken_fix = df_eye.copy()
                        df_broken_fix = df_broken_fix[df_broken_fix.In_Bound == 1] 
            ...
            #   for idx_annulus,operator in enumerate(['<','>=']):
                ...
                Data = zip(stim_intensities,n_correct,n_trials) #concatenates values together
                ...
                # (and how many trials they completed at that intensity) is saved
                rm_Data = zip(stim_intensities, rm_n_correct, rm_n_trials) # concatenates values together
            ...
            #    labelit = p['annulus_ori']
                ...
                for idx,this in enumerate(Data):
                    if n_trials>=3:
                        ...
                        #Contrast values:
                        x = np.hstack([x,this[2] * [this[0]]]) #horizontally stacking (a version of cat?)
                        #% correct:
                ...
                trial_history = pd.DataFrame(dict(x=trials, y=correct, labels=labels)) #plots trial history
                #fig1 = sns.lmplot("x", "y", hue="labels", data=trial_history, fit_reg=False)
                #ax_r.plot(np.mean(trial_history.x),np.mean( trial_history.y), '.')
                pcorrect, = ax_r.bar(1,len(trial_history.y[trial_history.y==1])/len(trial_history.y), width = .3,color = 'g')
                pincorrect, = ax_r.bar(0,len(trial_history.y[trial_history.y==0])/len(trial_history.y), width = .3, color = 'r')
    ...
    fig.savefig('%s.png'%(dir_name + '/proccessed/' + subject+'_'+date+ '_session' + session + '_FittedThresholdsWeibull'))
    fig.show()
    ...
    rm_fig.savefig('%s.png'%(dir_name + '/proccessed/' + subject+'_'+date+ '_session' + session + '_RemovedFittedThresholdWeibull'))
    rm_fig.show()

    fig_result.text(0.5, 0.04, 'Correct or Incorrect', ha='center')
    fig_result.text(0.04, 0.5, 'Proportion', va='center', rotation='vertical')
    fig_result.savefig(dir_name + '/proccessed/' + subject+'_'+date+ '_session' + session + '_BehavioralPerformance.png')
    fig_result.set_size_inches(12.5, 9.5)
    fig_result.show()

    fig_cont.text(0.5, 0.04, 'trial number', ha='center') # figure handle
    fig_cont.text(0.04, 0.5, 'Target Contrast', va='center', rotation='vertical')
    fig_cont.savefig(dir_name + '/proccessed/' + subject+'_'+date + '_session' + session + '_TargetContrast.png')
...
if __name__ =="__main__":
    ...
    flake = 0.99
    slope = 3.5
    plot_psychophysical_performance(current_subject, date, sub_folder, num_blocks) #only uses this function
    all_runs = sub_folder+'/proccessed/'+current_subject+'_'+date+'_session' + session+'_master_file.xlsx'

...
def plot_psychophysical_performance(subject, date, dir_name, num_blocks):
    ...
    output = open(dir_name + '/proccessed/' + subject+'_'+date+ '_session' + session + '_no_eye_data.txt', 'a') # Save Thresholds into a .txt file
    ...
    #   for idx_annulus,operator in enumerate(['<','>=']):
        ...
        cond_n_correct = [len(np.where(hit_amps==i)[0]) for i in cond_stim_intensities]
        cond_n_trials = [len(np.where(all_amps==i)[0]) for i in cond_stim_intensities]
        ...
        h_n_correct = [len(np.where(hit_amps==i)[0]) for i in h_stim_intensities]
        h_n_trials = [len(np.where(all_amps==i)[0]) for i in h_stim_intensities]
        ...
        v_n_correct = [len(np.where(hit_amps==i)[0]) for i in v_stim_intensities]
        v_n_trials = [len(np.where(all_amps==i)[0]) for i in v_stim_intensities]
    ...
    #    labelit = p['annulus_ori'] 
        ...
        output.write(cond + ' Tresh: %s, CI: [%s, %s];  \n'%(keep_th, lower, upper))
        output.write(cond + ' Horizontal Tresh: %s, CI: [%s, %s];  \n'%(hkeep_th, hlower, hupper))
        output.write(cond + ' Vertical Tresh: %s, CI: [%s, %s];  \n'%(vkeep_th, vlower, vupper))
        new_row = pd.DataFrame({'condition':[cond], 'surround':[surround], 'annulus': [annulus], 'surround_contrast':[surround_contrast], 'threshold_est':[keep_th]})
    ...
    fig.savefig('%s.png'%(dir_name + '/proccessed/' + subject+'_'+date+ 'combined_threshold_fit'))
    fig.show()
